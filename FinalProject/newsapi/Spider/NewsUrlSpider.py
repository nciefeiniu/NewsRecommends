...
def urlcollect(lid):
    ...
    # 并把处理号的URL导入到数据库中储存
    changedict = {"2518": 0, "2510": 1, "2511": 2, "2669": 3, "2512": 4, "2513": 5, "2514": 6, "2515": 7, "2516": 8,
                  "2517": 9}
    ...
    for numbers in range(len(urls)):
        ...
        sql_i = "INSERT INTO news_api_urlcollect(url, type, time) values ('%s', '%d', '%s')" % (
        urls[numbers], Type, time)
...
def begincollect(time):
    ...
    try:
        ...
        sched.add_job(urlcollect, 'interval', max_instances=1, seconds=time, id='urlcollect10',
                      kwargs={"lid": "2518", })
        # urlcollect(lid)
